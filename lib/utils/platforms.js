...
export function detectPlatform(platform) {
  ...
  if (name.includes("64") || name.includes("x64") || name.includes("amd64"))
    suffix = "64";
  if (name.includes("arm64") || name.includes("arm")) suffix = "arm64";
...
export function resolveForVersion(version, platformID, opts) {
  ...
  const formattedPlatformID = detectPlatform(platformID);

  return version.platforms
    .filter((pl) => pl.type.indexOf(formattedPlatformID) === 0)
    .sort((p1, p2) => {
      let result = 0;
