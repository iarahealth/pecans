...
export class Pecans {
  ...
  async handleDownload(req, res, next) {
    let channel = req.params.channel || "stable";
    ...
    try {
      ...
      if (!asset)
        throw new Error(
          `No download available for platform ${platform} for version ${
            version.tag
          } (${channel || "beta"})`
        );
  ...
  async handleUpdate(req, res, next) {
    try {
      ...
      versions = versions.filter(
        (version) =>
          !version.tag.startsWith("0.12.4") &&
          !version.tag.startsWith("0.13.10")
      );
      if (versions.length === 0) return res.status(204).send("No updates");

      const latest = versions[0];
      console.log("debug handleUpdate:", tag, latest.tag, platform);
  ...
  async handleUpdateWin(req, res, next) {
    try {
      ...
      // Update needed?
      const latest = versions[0];
      console.log("debug handleUpdateWin:", tag, latest.tag, platform);
